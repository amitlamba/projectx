...
def addDataToExcel(companyName,companyShortForm,findthis): #for Reliance 
    ...
    hist_folder = os.path.join(output_directory, "hist_Ratios")
    mControl = os.path.join(hist_folder, "MoneyControl")
    if not os.path.exists(mControl):
            os.makedirs(mControl)
       
    output_filename="HR_"+companyName+"_using_MoneyControl.xlsx"
    excel_file_path = os.path.join(mControl, output_filename)
